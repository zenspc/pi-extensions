...
This skill orchestrates three others: an inline mining pass (see step 1), `playbooks/authoring-a-skill.md` (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Use `playbooks/authoring-a-skill.md` skill to author the skill. Placement:
...
- Frontmatter formatting: follow `playbooks/authoring-a-skill.md`'s YAML rules. Keep `description` as one YAML scalar. Quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Apply the **unslop** skill and `playbooks/authoring-a-skill.md`'s writing guidelines to every line.
...
A `-mode` skill is subjective output. A `playbooks/authoring-a-skill.md`-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.
