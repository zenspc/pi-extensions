...
- Prefer ``subagent({ agent: "poteto-agent", task: ... })``. The `worker` builtin is the fallback.
- Pass `model:` explicitly per the configured roles (default `inherit-parent`; omit `model`).
...
- Browser / Electron / Web UIs: a browser-automation harness (Playwright, CDP) or the project's verification skill.
- CLIs and TUIs: spawn the binary in a pty and drive it, or use the project's verification skill.
