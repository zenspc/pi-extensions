...
**PRs.** Run `/skill:deslop` over the branch diff before commit. Run `/skill:unslop` over prose surfaces of the diff. Run `/skill:no-comments` before review. Write every PR title, PR description, and commit body with `/skill:technical-writing`, then apply `/skill:unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.
